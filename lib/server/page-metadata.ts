...
async function fetchSiebelPagesMeta(): Promise<Record<string, unknown>> {
...
export async function generateSiebelPagesMetadata(canonicalPath: string): Promise<Metadata> {
  ...
  const title = (data?.MetaTitle as string) || FALLBACK_TITLE;
  const description = (data?.MetaDescriptions as string) || "";
  const keywords = (data?.MetaKeywords as string) || "";
