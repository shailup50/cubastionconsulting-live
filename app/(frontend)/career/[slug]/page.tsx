...
const IMAGE_BASE_URL = process.env.NEXT_PUBLIC_IMAGE_URL || "https://cubastionapi.cyralix.com/uploads/onlineImages/CareerImages";

async function fetchCareerData(slug: string) {
    try {
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    try {
        ...
        const imageUrl = data.CareerBannerImage
            ? `${IMAGE_BASE_URL}/${data.CareerBannerImage}`
            : "/OGImage/career-og.jpg";
...
export default async function CareerDetailPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    if (!careerData) {
        return <div className="sec-pad-all"><div className="container">Career Not Found</div></div>;
    }
