...
export default function ManageTeam() {
  ...
  const { data: teamData, isLoading, isFetching, refetch } = useGetAllTeamsQuery(undefined, { refetchOnMountOrArgChange: true });
      ...
      cell: (row: any, index: number) => (
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <div style={{ width: "40px", height: "40px", flexShrink: 0 }}>
            {row.TeamImage ? (
              <img
                src={`/uploads/onlineImages/TeamImages/${row.TeamImage}`}
                alt="Team"
    ...
    },
    {
      ...
      cell: (row: any) => (
        <div style={{ display: "flex", gap: "10px" }}>
          <Link href={`/cubastion-admin/add-team?ID=${row.TeamID}`} className="edit-icon">
          ...
          <button onClick={() => handleDelete(row.TeamID)} className="approve-btn" style={{ color: "#dc3545", padding: "0" }}>
  ...
  const subHeaderComponent = useMemo(() => (
    <div className="subheader-container">
      ...
      <div className="colB">
        <Link href="/cubastion-admin/add-team" className="addnew-btn" style={{ width: "110px" }}>
          <span>+</span> Add New
        </Link>
      </div>
    </div>
  ...
  const tableData = useMemo(() => {
    const rawData = teamData?.data || [];
    return rawData.filter((item: any) => {
      const searchText = filterText.toLowerCase();
      return (
        item.TeamName?.toLowerCase().includes(searchText) ||
        item.TeamDesignation?.toLowerCase().includes(searchText) ||
        item.TeamType?.toLowerCase().includes(searchText)
      );
    });
  }, [teamData, filterText]);
  ...
  return (
    <main>
      <DataTable
        ...
        responsive
        noDataComponent={<div style={{ padding: '15px', textAlign: 'center' }}>{teamData?.message || "No data available"}</div>}
