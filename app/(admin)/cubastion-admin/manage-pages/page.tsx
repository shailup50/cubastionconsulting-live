...
export default function ManagePages() {
  ...
  const { data: pageData, isLoading, isFetching, refetch } = useGetAllPagesQuery(undefined, { refetchOnMountOrArgChange: true });
  ...
  const handleDelete = async (id: any) => {
    ...
    try {
      ...
      if (res.status || res.success) {
        toast.success(res.message || "Page deleted successfully");
        refetch();
      } else {
  ...
  const columns = useMemo(() => [
    {
      name: "Page Name",
      selector: (row: any) => row.StaticPageName,
      sortable: true,
      cell: (row: any, index: number) => (
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            <span style={{ fontWeight: 600 }}>{row.StaticPageName}</span>
          </div>
        </div>
      ),
      width: "85%",
    ...
    },
    {
      ...
      cell: (row: any) => (
        <div style={{ display: "flex", gap: "10px" }}>
          <Link href={`/cubastion-admin/add-pages?ID=${row.StaticPageID}`} className="edit-icon">
          ...
          {/* <button onClick={() => handleDelete(row.StaticPageID)} className="approve-btn" style={{ color: "#dc3545", padding: "0" }}>
  ...
  const subHeaderComponent = useMemo(() => (
    <div className="subheader-container">
      ...
      <div className="colB">
        <Link href={"/cubastion-admin/add-pages"} className="addnew-btn" style={{ width: "110px" }}>
          <span>+</span> Add New
        </Link>
      </div>
    </div>
  ...
  const tableData = useMemo(() => {
    ...
    return rawData.filter((item: any) => {
      ...
      return (
        item.StaticPageName?.toLowerCase().includes(searchText) ||
        item.StaticPageNameURL?.toLowerCase().includes(searchText)
      );
    });
  }, [pageData, filterText]);
  ...
  return (
    <main>
      <DataTable
        ...
        responsive
        noDataComponent={<div style={{ padding: '15px', textAlign: 'center' }}>{pageData?.message || "No data available"}</div>}
