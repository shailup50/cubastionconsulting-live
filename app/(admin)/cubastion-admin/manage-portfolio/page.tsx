...
export default function ManagePortfolio() {
  ...
  const { data: portfolioData, isLoading, isFetching, refetch } = useGetAllPortfoliosQuery(undefined, { refetchOnMountOrArgChange: true });
  ...
  const columns = useMemo(() => [
    {
      name: "Portfolio Item",
      selector: (row: any) => row.PortfolioName,
      sortable: true,
      cell: (row: any, index: number) => (
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <div className="user-image" style={{ width: "40px", height: "40px", flexShrink: 0 }}>
            {row.PortfolioImage ? (
              <img
                src={`/uploads/onlineImages/PortfolioImages/${row.PortfolioImage}`}
                alt="Portfolio"
                style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "4px", backgroundColor: "#f8f9fa" }}
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            <span style={{ fontWeight: 600 }}>{row.PortfolioName}</span>
            <small style={{ color: "#666" }}>{row.PortfolioType || "General"}</small>
          </div>
        </div>
      ),
      width: "84%",
    ...
    },
    {
      ...
      cell: (row: any) => (
        <div style={{ display: "flex", gap: "10px" }}>
          <Link href={`/cubastion-admin/add-portfolio?ID=${row.PortfolioID}`} className="edit-icon">
          ...
          <button onClick={() => handleDelete(row.PortfolioID)} className="approve-btn" style={{ color: "#dc3545", padding: "0" }}>
  ...
  const subHeaderComponent = useMemo(() => (
    <div className="subheader-container">
      ...
      <div className="colB">
        <Link href={"/cubastion-admin/add-portfolio"} className="addnew-btn" style={{ width: "110px" }}>
          <span>+</span> Add New
        </Link>
      </div>
    </div>
  ...
  const tableData = useMemo(() => {
    const rawData = portfolioData?.data || [];
    return rawData.filter((item: any) => {
      const searchText = filterText.toLowerCase();
      return (
        item.PortfolioName?.toLowerCase().includes(searchText) ||
        item.PortfolioType?.toLowerCase().includes(searchText) ||
        item.PortfolioNameURL?.toLowerCase().includes(searchText)
      );
    });
  ...
  return (
    <main>
      <DataTable
        ...
        responsive
        noDataComponent={<div style={{ padding: '15px', textAlign: 'center' }}>{portfolioData?.message || "No data available"}</div>}
