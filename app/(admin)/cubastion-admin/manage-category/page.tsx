...
export default function ManageCategories() {
  ...
  const [displayOrders, setDisplayOrders] = useState<Record<string, any>>({});
  ...
  const handleDisplayOrderChange = useCallback((id: any, value: string) => {
    setDisplayOrders(prev => ({ ...prev, [id]: value === "" ? "" : parseInt(value, 10) }));
  }, []);

  const handleApiAction = async (actionPromise: any, successMsg: string, onSuccessCallback?: () => void) => {
    try {
  ...
  const handleUpdateDisplayOrder = async () => {
    const changedEntries = Object.entries(displayOrders).filter(([id, val]) => {
      const original = categoryData?.data?.find((c: any) => c.CategoryID === parseInt(id))?.DisplayOrder;
      return val != original;
    });
    ...
    const actionPromise = Promise.all(changedEntries.map(([id, val]) => {
      const item = categoryData.data.find((c: any) => c.CategoryID === parseInt(id));
  ...
  const handleStatusUpdate = async (row: any, targetStatus: any) => {
    ...
    },
    {
      ...
      cell: (row: any) => (
        <input
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleDisplayOrderChange(row.CategoryID, e.target.value)}
