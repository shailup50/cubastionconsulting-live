...
export default function ManageLogos() {
  ...
  const { data: logoData, isLoading, isFetching, refetch } = useGetAllLogosQuery(undefined, { refetchOnMountOrArgChange: true });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await deleteLogo(id).unwrap();
    ...
    },
    {
      ...
      cell: (row) => (
        <div style={{ display: "flex", gap: "10px" }}>
          <Link href={`/cubastion-admin/add-logo?ID=${row.LogoID}`} className="edit-icon">
  ...
  const subHeaderComponent = useMemo(() => (
    <div className="subheader-container">
      ...
      <div className="colB">
        <Link href={"/cubastion-admin/add-logo"} className="addnew-btn" style={{ width: "110px" }}>
          <span>+</span> Add New
        </Link>
      </div>
    </div>
  ...
  return (
    <main>
      <DataTable
        ...
        responsive
        noDataComponent={<div style={{ padding: '15px', textAlign: 'center' }}>{logoData?.message || "No data available"}</div>}
