...
export default function ManageIndustries() {
  ...
  const { data: industryData, isLoading, isFetching, refetch } = useGetAllIndustriesQuery(undefined, { refetchOnMountOrArgChange: true });
  ...
  const handleDelete = async (id: any) => {
    ...
    try {
      const res = await deleteIndustry(id).unwrap();
      if (res.status || res.success) {
        toast.success(res.message || "Industry deleted successfully");
        refetch();
      } else {
  ...
  const columns = useMemo(() => [
    {
      name: "Industry Name",
      selector: (row: any) => row.IndustryName,
      sortable: true,
      cell: (row: any, index: number) => (
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <div className="user-image" style={{ width: "40px", height: "40px", flexShrink: 0 }}>
            {row.IndustryImage ? (
              <img
                src={`/uploads/onlineImages/IndustryImages/${row.IndustryImage}`}
                alt="Industry"
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            <span style={{ fontWeight: 600 }}>{row.IndustryName}</span>
            <small style={{ color: "#666" }}>{row.IndustryNameURL}</small>
          </div>
        </div>
      ),
      width: "88%",
    ...
    },
    {
      ...
      cell: (row: any) => (
        <div style={{ display: "flex", gap: "10px" }}>
          <Link href={`/cubastion-admin/add-industries?ID=${row.IndustryID}`} className="edit-icon">
          ...
          <button onClick={() => handleDelete(row.IndustryID)} className="approve-btn" style={{ color: "#dc3545", padding: "0" }}>
  ...
  const subHeaderComponent = useMemo(() => (
    <div className="subheader-container">
      ...
      <div className="colB">
        <Link href={"/cubastion-admin/add-industries"} className="addnew-btn" style={{ width: "110px" }}>
          <span>+</span> Add New
        </Link>
      </div>
    </div>
  ...
  const tableData = useMemo(() => {
    const rawData = industryData?.data || [];
    return rawData.filter((item: any) => {
      const searchText = filterText.toLowerCase();
      return (
        item.IndustryName?.toLowerCase().includes(searchText) ||
        item.IndustryNameURL?.toLowerCase().includes(searchText)
      );
    });
  }, [industryData, filterText]);
  ...
  return (
    <main>
      <DataTable
        ...
        responsive
        noDataComponent={<div style={{ padding: '15px', textAlign: 'center' }}>{industryData?.message || "No data available"}</div>}
