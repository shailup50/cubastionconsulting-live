...
export default function ManageTestimonials() {
  ...
  const { data: testimonialData, isLoading, isFetching, refetch } = useGetAllTestimonialsQuery(undefined, { refetchOnMountOrArgChange: true });
  ...
  const columns = useMemo(() => [
    {
      name: "Client",
      selector: (row: any) => row.TestimonialName,
      sortable: true,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <div className="user-image" style={{ width: "40px", height: "40px", flexShrink: 0 }}>
            {row.TestimonialImage ? (
              <img
                src={`/uploads/onlineImages/TestimonialImages/${row.TestimonialImage}`}
                alt="Client"
                style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: "50%", backgroundColor: "#f8f9fa" }}
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            <span style={{ fontWeight: 600 }}>{row.TestimonialName}</span>
          </div>
        </div>
      ),
      width: "83%",
    ...
    },
    {
      ...
      cell: (row: any) => (
        <div style={{ display: "flex", gap: "10px" }}>
          <Link href={`/cubastion-admin/add-testimonials?ID=${row.TestimonialID}`} className="edit-icon">
          ...
          <button onClick={() => handleDelete(row.TestimonialID)} className="approve-btn" style={{ color: "#dc3545", padding: "0" }}>
  ...
  const subHeaderComponent = useMemo(() => (
    <div className="subheader-container">
      ...
      <div className="colB">
        <Link href="/cubastion-admin/add-testimonials" className="addnew-btn" style={{ width: "110px" }}>
          <span>+</span> Add New
        </Link>
      </div>
    </div>
  ...
  const tableData = useMemo(() => {
    const rawData = testimonialData?.data || [];
    return rawData.filter((item: any) => {
      const searchText = filterText.toLowerCase();
      return item.TestimonialName?.toLowerCase().includes(searchText);
    });
  }, [testimonialData, filterText]);
  ...
  return (
    <main>
      <DataTable
        ...
        responsive
        noDataComponent={<div style={{ padding: '15px', textAlign: 'center' }}>{testimonialData?.message || "No data available"}</div>}
