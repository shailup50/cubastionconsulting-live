...
export default function ManageAuthors() {
  ...
  const { data: authorData, isLoading, isFetching, refetch } = useGetAllAuthorsQuery(undefined, { refetchOnMountOrArgChange: true });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await deleteAuthor(id).unwrap();
      ...
      cell: (row, index) => (
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <div style={{ width: "40px", height: "40px", flexShrink: 0 }}>
            {row.AuthorImage ? (
              <img
                src={`/uploads/onlineImages/AuthorImages/${row.AuthorImage}`}
                alt="Author"
                style={{ width: "40px", height: "40px", objectFit: "cover", borderRadius: "50%", backgroundColor: "#f8f9fa" }}
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            <span style={{ fontWeight: 600 }}>{row.AuthorName}</span>
          </div>
        </div>
      ),
      width: "85%",
    ...
    },
    {
      ...
      cell: (row) => (
        <div style={{ display: "flex", gap: "10px" }}>
          <Link href={`/cubastion-admin/add-author?ID=${row.AuthorID}`} className="edit-icon">
          ...
          <button onClick={() => handleDelete(row.AuthorID)} className="approve-btn" style={{ color: "#dc3545", padding: "0" }}>
  ...
  const subHeaderComponent = useMemo(() => (
    <div className="subheader-container">
      ...
      <div className="colB">
        <Link href="/cubastion-admin/add-author" className="addnew-btn" style={{ width: "110px" }}>
          <span>+</span> Add New
        </Link>
      </div>
    </div>
  ...
  const tableData = useMemo(() => {
    const rawData = authorData?.data || [];
    return rawData.filter((item) => {
      const searchText = filterText.toLowerCase();
      return (
        item.AuthorName?.toLowerCase().includes(searchText) ||
        item.AuthorTaglin?.toLowerCase().includes(searchText)
      );
    });
  }, [authorData, filterText]);
  ...
  return (
    <main>
      <DataTable
        ...
        responsive
        noDataComponent={<div style={{ padding: '15px', textAlign: 'center' }}>{authorData?.message || "No data available"}</div>}
