...
import { useGetAllCareersQuery, useDeleteCareerMutation } from "@/store/backendSlice/careerAPISlice";
...
export default function ManageCareers() {
  ...
  const { data: careersResponse, isLoading, isFetching, refetch } = useGetAllCareersQuery(undefined, { refetchOnMountOrArgChange: true });
  ...
  const columns = useMemo(() => [
    {
      name: "Career Details",
      selector: (row: any) => row.CareerName,
      sortable: true,
      cell: (row: any, index: number) => (
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <div className="user-image" style={{ width: "40px", height: "40px", flexShrink: 0 }}>
            {row.CareerImage ? (
              <img
                src={`/uploads/onlineImages/ServiceImages/${row.CareerImage}`}
                alt="Career"
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            <span style={{ fontWeight: 600 }}>{row.CareerName}</span>
            <span style={{ fontSize: "0.8rem", color: "#666" }}>{row.JobCategoryName} - {row.CareerPosition}</span>
          </div>
        </div>
      ),
      width: "80%",
    ...
    {
      name: "Location",
      selector: (row: any) => row.CareerLocation,
      sortable: true,
      width: "8%",
    },
    {
    ...
    },
    {
      ...
      cell: (row: any) => (
        <div style={{ display: "flex", gap: "10px" }}>
          <Link href={`/cubastion-admin/add-careers?ID=${row.CareerID}`} className="edit-icon">
  ...
  const subHeaderComponent = useMemo(() => (
    <div className="subheader-container">
      ...
      <div className="colB">
        <Link href={"/cubastion-admin/add-careers"} className="addnew-btn" style={{ width: "110px" }}>
          <span>+</span> Add New
        </Link>
      </div>
    </div>
  ...
  const tableData = useMemo(() => {
    const rawData = careersResponse?.data || [];
    return rawData.filter((item: any) => {
      const searchText = filterText.toLowerCase();
      return (
        item.CareerName?.toLowerCase().includes(searchText) ||
        item.JobCategoryName?.toLowerCase().includes(searchText) ||
        item.CareerLocation?.toLowerCase().includes(searchText)
      );
    });
  }, [careersResponse, filterText]);
  ...
  return (
    <main>
      <DataTable
        ...
        responsive
        noDataComponent={<div style={{ padding: '15px', textAlign: 'center' }}>{careersResponse?.message || "No data available"}</div>}
