...
export default function ManageCareerApplications() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetAllCareerApplicationsQuery(undefined, {
    refetchOnMountOrArgChange: true,
  });
  const [deleteApp] = useDeleteCareerApplicationMutation();

  const openAppPopup = (app: any) => {
    setSelectedApp(app);
    setIsPopupOpen(true);
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) =>
        row.Resume ? (
          <a
            href={`/uploads/onlineImages/CareerImages${row.Resume}`}
            target="_blank"
    ...
    },
    {
      ...
      cell: (row) => (
        <button
          onClick={() => handleDelete(row.CareerID)}
          className="approve-btn"
    ...
    <div className="subheader-container">
      <div className="colA">
        ...
        >
          ...
          {Array.from(
            new Set(
              (appResponse?.data || []).map((item) => item.JobName || ""),
            ),
          )
            .filter((job) => job)
            .sort()
      ...
      {isPopupOpen && selectedApp && (
        ...
        >
          ...
          >
            ...
            >
              Application from {selectedApp.FullName}
            </h2>
            <div
              style={{ display: "flex", flexDirection: "column", gap: "10px" }}
            >
              <p>
                <strong>Job applied for:</strong> {selectedApp.JobName || "N/A"}
              </p>
              <p>
                <strong>Email:</strong> {selectedApp.Email}
              </p>
              <p>
                <strong>Phone:</strong> {selectedApp.PhoneNo}
              </p>
              <p>
                <strong>Location:</strong> {selectedApp.City},{" "}
                {selectedApp.Country}
              </p>
              <p>
                <strong>LinkedIn:</strong>{" "}
                <a
                  href={selectedApp.LinkedInLink}
                  target="_blank"
                  rel="noreferrer"
                  style={{ color: "#0a66c2", textDecoration: "underline" }}
                >
                  {selectedApp.LinkedInLink}
                </a>
              </p>
              <p>
                <strong>Posted Date:</strong>{" "}
                {formatDate(selectedApp.PostedDate)}
              </p>
              <hr
                style={{
                  border: "0",
