...
export async function GET(req, { params }) {
  try {
    ...
    const service = await queryOne('SELECT * FROM mst_servicedata WHERE ServiceID = ?', [id]);
    if (!service) return NextResponse.json({ status: false, message: 'Service not found' }, { status: 404 });
    const faqs = await query('SELECT * FROM mst_servicefaqdata WHERE ServiceID = ?', [id]);
    ...
    return NextResponse.json({ status: false, message: 'Error fetching service', error: error.message }, { status: 500 });
  }
}
...
export async function PUT(req, { params }) {
  try {
    ...
    const updated = await queryOne('SELECT * FROM mst_servicedata WHERE ServiceID = ?', [id]);
    return NextResponse.json({ status: true, message: 'Service updated successfully', data: updated });
  } catch (error) {
    return NextResponse.json({ status: false, message: 'Something went wrong while updating service', error: error.message }, { status: 500 });
  }
}
...
export async function DELETE(req, { params }) {
  try {
    ...
    return NextResponse.json({ status: false, message: 'Failed to delete service', error: error.message }, { status: 500 });
  }
}
