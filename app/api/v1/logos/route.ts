...
export async function GET(_request: NextRequest) {
  try {
    const logos = await query<Logo>(
      'SELECT * FROM mst_logodata WHERE ActiveStatus IN (0, 1) ORDER BY LogoID DESC',
    );
    return NextResponse.json({ status: true, data: logos });
  } catch (error) {
    return NextResponse.json({ status: false, message: 'Failed to fetch logos', error: (error as Error).message }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const logo = await query<Logo>('SELECT * FROM mst_logodata WHERE LogoID = ?', [result.insertId]);
    return NextResponse.json({ status: true, message: 'Logo created successfully', data: logo[0] }, { status: 201 });
  } catch (error) {
    return NextResponse.json({ status: false, message: 'Something went wrong while creating logo', error: (error as Error).message }, { status: 500 });
  }
}
