...
export async function GET() {
  try {
    const logos = await query('SELECT * FROM mst_logodata ORDER BY LogoID DESC');
...
export async function POST(req) {
  try {
    ...
    const existing = await query('SELECT LogoID FROM mst_logodata WHERE LogoNameURL = ?', [LogoNameURL]);
