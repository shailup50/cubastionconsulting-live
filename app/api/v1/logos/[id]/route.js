...
export async function PUT(req, { params }) {
  try {
    ...
    const updated = await queryOne('SELECT * FROM mst_logodata WHERE LogoID = ?', [id]);
    return NextResponse.json({ status: true, message: 'Logo updated successfully', data: updated });
  } catch (error) {
    return NextResponse.json({ status: false, message: 'Something went wrong while updating logo', error: error.message }, { status: 500 });
  }
}
...
export async function DELETE(req, { params }) {
  try {
    ...
    return NextResponse.json({ status: false, message: 'Failed to delete logo', error: error.message }, { status: 500 });
  }
}
