...
function Opportunities({ careers = [], id }: any) {
  const swiperRef = useRef(null);

  const sectionHeading = "Opportunities That Help You Grow";

  const mappedCareers = careers.map((career: any, index: number) => ({
            ...
            <div className="btn-group">
              ...
              <div className="swiper-nav group">
                <SwiperButton
                  ...
                  onClick={() => (swiperRef as any).current?.slidePrev()}
                />
                <SwiperButton
                  classname="swiper-next"
                  onClick={() => (swiperRef as any).current?.slideNext()}
                />
              </div>
            </div>
          </div>
