...
export default function CareerPage({ initialCareersResponse = null }: any) {
  ...
  const activeCareers = (careersApiResponse?.data || []).filter(
    (career: any) => Number(career.ActiveStatus) === 1,
  );
  ...
  return (
    <main>
      {activeCareers.length > 0 && (
        <HeroSection id="heroSec" careers={activeCareers} />
      )}
      {ClientSucessData && (
      ...
      {activeCareers.length > 0 && (
        <Opportunities id="opportunities" careers={activeCareers} />
      )}
