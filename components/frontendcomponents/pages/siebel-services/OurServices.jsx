              ...
              {tabs.map((tab, index) => {
                ...
                return (
                  <button
                    ...
                    className={`relative flex min-h-[46px] shrink-0 items-center justify-center gap-2 px-2! py-3! text-center text-[14px]! md:text-base! font-medium! transition-all duration-300 sm:px-3! sm:text-sm! lg:min-h-[52px] lg:whitespace-nowrap lg:px-2! ${
                      isActive
                        ? "text-[#052559]"
            ...
            >
              {activeTab.items.map((item) => {
                ...
                return (
                  <article
                    key={item.id}
                    className="min-h-[160px] rounded-[16px] border border-primary-start border-t-[4px] border-t-[#052559] bg-white p-4! shadow-[0_10px_24px_rgba(17,24,39,0.05)] sm:min-h-[170px] sm:rounded-[18px] sm:p-5!"
                  >
