...
function validateForm(values: { name: string; email: string; idea: string }) {
...
interface TechCoFounderContactFormProps {
  formConfig: any;
  headingTag?: "h2" | "h3";
  ideaRows?: number;
}
...
export function TechCoFounderContactForm({
  ...
  const fieldClass = (field: string) => (errors[field] ? inputErrorClass : inputClass);
