...
export default function JobDetailsPage({ slug, initialData, initialCareersResponse = null }: any) {
    ...
    const activeCareers = (careersApiResponse?.data || [])
        .filter((c: any) => Number(c.ActiveStatus) === 1 && c.CareerNameURL !== slug);

    const heroData = {
        heading: jobData?.CareerName || (staticData as any).JobDetails.Section1.heading,
    ...
    const jobDetailsData = jobData || (staticData as any).JobDetails.Section2
    const similarDataTitle = (staticData as any).JobDetails.Section3.heading
    const positionData = (staticData as any).JobDetails.Section4
