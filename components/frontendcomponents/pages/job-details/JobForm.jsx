...
export default function JobForm({ jobName }) {
    ...
    const handleFileChange = (e, name) => {
        ...
        setSelectedFiles(prev => ({ ...prev, [name]: newFile }));
        handleInput(name, "/" + newFile.name); // Using the /filename format as requested
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Step 1: Upload to local server via FormData to get the clean filename
            if (selectedFiles.resume) {
                ...
                if (localRes.data.success) {
                    finalResumeName = `/${localRes.data.fileName}`; // Explicitly prefixing with /
                } else {
                    throw new Error("Local resume upload failed");
                }
            }
            // Step 2: Final submission to the main career API via AXIOS JSON
            const payload = {
