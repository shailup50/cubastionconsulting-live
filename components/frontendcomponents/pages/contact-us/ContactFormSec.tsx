...
export default function ContactFormSec({ id }: any) {
  const dispatch = useDispatch<any>();
  const { headerPayload } = useHeaderData() as any;
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const response = await (dispatch as any)(createContactData(formData)).unwrap();
              ...
              <div className="form-grid">
                ...
                <div style={{ position: "relative", paddingBottom: "20px" }}>
                  <Select
                    ...
                    onChange={handleChange}
                    options={industriesList?.map((industry: any) => ({
